...
const RegisterPage = () => {
  ...
  const roles = [
    { value: 'ATTENDEE', label: 'Attendee', icon: <TicketIcon className="w-7 h-7" />, description: 'Join amazing events and experiences' },
    { value: 'ORGANIZER', label: 'Organizer', icon: <MicrophoneIcon className="w-7 h-7" />, description: 'Create and manage your events' },
    { value: 'STAFF', label: 'Staff', icon: <UserGroupIcon className="w-7 h-7" />, description: 'Help manage event operations' },
  ];
  ...
  const handleSubmit = async (values: FormValues, { setSubmitting, setStatus }: { setSubmitting: (isSubmitting: boolean) => void; setStatus: (status: any) => void }) => {
    try {
      ...
      const registerData = {username: values.username, email: values.email, password: values.password, role: values.role};
              ...
              >
                {({ isSubmitting, setFieldValue, status }) => (
                  <Form className="space-y-6">
                    ...
                    <div>
                      <label htmlFor="username" className="block text-l font-medium text-gray-700 mb-2">
                        Username
                      </label>
                        ...
                        {roles.map((role) => (
                          <motion.div
                            ...
                            className={`p-4 border rounded-lg cursor-pointer transition-all ${
                              selectedRole === role.value ? 'border-purple-500 bg-purple-50' : 'border-gray-200 hover:border-purple-300'
                            }`}
                          >
                            <div className="flex flex-col items-center text-center">
                              <div
                                className={`p-3 rounded-full mb-3 ${
                                  selectedRole === role.value ? 'bg-purple-100 text-purple-600' : 'bg-gray-100 text-gray-600'
                                }`}
                              >
                                {role.icon}
                              </div>
                              <span className="font-medium text-base text-gray-800">{role.label}</span>
                    ...
                    <div className="flex items-center">
                      ...
                      />
                      <label htmlFor="terms" className="ml-2 block text-base text-gray-700 flex items-center">
                        <ShieldCheckIcon className="h-6 w-6 text-gray-400 mr-2" />
                        I agree to the{' '}
                        <a href="/terms" className="text-purple-600 hover:text-purple-500 mx-1">
                          Terms
                        </a>{' '}
                        and{' '}
                        <a href="/privacy-policy" className="text-purple-600 hover:text-purple-500 mx-1">
                          Privacy Policy
                        </a>
                      </label>
                      <ErrorMessage name="terms" component="div" className="text-red-500 text-sm mt-1 w-full" />
                    </div>
                    ...
                    >
                      {isSubmitting ? (
                        ...
                        <>
                          Register Now <ArrowRightIcon className="w-6 h-6 ml-2" />
                        </>
                      )}
                    </motion.button>
